...
namespace org.bidib.nbidibc.Simulation.Models.Nodes
{
    public class GbmBoostNode : SimulationNode
    {
                ...
                case BiDiBMessage.MSG_BOOST_ON:
                    {
                        ...
                        addResponse.Invoke(BiDiBMessageGenerator.GenerateMessage(Address, BiDiBMessage.MSG_BOOST_STAT, message.SequenceNumber, (byte)BoosterState.BIDIB_BST_STATE_ON));
                ...
                case BiDiBMessage.MSG_BOOST_OFF:
                    {
                        ...
                        addResponse.Invoke(BiDiBMessageGenerator.GenerateMessage(Address, BiDiBMessage.MSG_BOOST_STAT, message.SequenceNumber, (byte)BoosterState.BIDIB_BST_STATE_OFF));
                        if (message.MessageParameters[0] == 0)
                        {
                            ForwardToChildren(message, addResponse);
                        }
        ...
        private async Task HandleCommandStationProgrammingOnMainMessageAsync(BiDiBInputMessage message)
        {
            ...
            AddResponse.Invoke(BiDiBMessageGenerator.GenerateMessage(Address, BiDiBMessage.MSG_CS_POM_ACK, message.SequenceNumber, parameters.ToArray()));
            ...
            AddResponse.Invoke(BiDiBMessageGenerator.GenerateMessage(Address, BiDiBMessage.MSG_BM_CV, message.SequenceNumber, parameters.ToArray()));
        }
        ...
        private async Task HandleCommandStationProgrammingMessageAsync(BiDiBInputMessage message)
        {
            ...
            await Task.Delay(500).ConfigureAwait(false);
            AddResponse.Invoke(BiDiBMessageGenerator.GenerateMessage(Address, BiDiBMessage.MSG_CS_PROG_STATE, message.SequenceNumber, parameters));
        }
        ...
        protected override void OnTriggerValueChange()
        {
            ...
            byte current = Convert.ToByte(StaticRandom.Instance.Next(0, Features.FirstOrDefault(x => x.FeatureType == BiDiBFeature.FEATURE_BST_AMPERE)?.Value ?? 250));
            byte voltage = Convert.ToByte(StaticRandom.Instance.Next(0, 253));
            byte temp = Convert.ToByte(StaticRandom.Instance.Next(0, 127));

            AddResponse.Invoke(BiDiBMessageGenerator.GenerateMessage(Address, BiDiBMessage.MSG_BOOST_DIAGNOSTIC, LastSequenceNumber++, 0x00, current, 0x01, voltage, 0x02, temp));
        }
